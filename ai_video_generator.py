...
Scene scripts are generated by a real LLM (see llm_client.py, needs
OPENAI_API_KEY); without a key it falls back to the static style mappings.
"""
...
try:
    ...
    print("Warning: librosa not available - using built-in WAV analyzer fallback")
import numpy as np
import json
...
try:
    ...
    # Placeholder names so class-level annotations don't NameError at import
    VideoFileClip = AudioFileClip = ImageClip = object
    concatenate_videoclips = CompositeVideoClip = TextClip = object
...
class AudioAnalyzer:
    ...
    def analyze(self, audio_path: str, segment_duration: float = 3.0) -> List[AudioSegment]:
        ...
        if LIBROSA_AVAILABLE:
            return self._analyze_with_librosa(audio_path, segment_duration)
        if audio_path.lower().endswith(".wav"):
            print("librosa missing: using built-in WAV fallback analyzer")
            return self._analyze_wav_fallback(audio_path, segment_duration)
        raise RuntimeError(
            "librosa is not installed and the built-in fallback only supports .wav files. "
            "Install librosa (pip install librosa) or convert the audio to WAV.")

    def _analyze_with_librosa(self, audio_path: str, segment_duration: float) -> List[AudioSegment]:
        # Load audio
    ...
    def _analyze_wav_fallback(self, audio_path: str, segment_duration: float) -> List[AudioSegment]:
        """Real signal analysis for 16-bit PCM WAV files using only stdlib + numpy.

        Computes per-segment RMS energy, spectral centroid (via FFT) and
        zero-crossing rate, then reuses the same heuristic emotion classifier.
        """
        with wave.open(audio_path, "rb") as w:
            n_channels, sampwidth, sr, n_frames = w.getnchannels(), w.getsampwidth(), w.getframerate(), w.getnframes()
            raw = w.readframes(n_frames)
        if sampwidth != 2:
            raise RuntimeError(f"WAV fallback needs 16-bit PCM, got {sampwidth * 8}-bit")
        ...
        for i in range(num_segments):
            start_time = i * segment_duration
            end_time = min((i + 1) * segment_duration, duration)
            seg = y[int(start_time * sr):int(end_time * sr)]
            if len(seg) == 0:
                continue
            energy = float(np.sqrt(np.mean(seg ** 2)))
            frames = [seg[j:j + frame] * np.hanning(min(frame, len(seg[j:j + frame])))
                      for j in range(0, max(1, len(seg) - frame), hop)] or [seg]
            mags = [np.abs(np.fft.rfft(f, n=frame)) for f in frames]
            freqs = np.fft.rfftfreq(frame, 1 / sr)
            centroids = [float(np.sum(m * freqs) / (np.sum(m) + 1e-9)) for m in mags]
            centroid = float(np.mean(centroids))
            zcr = float(np.mean([np.mean(np.abs(np.diff(np.signbit(f)))) for f in frames]))
            # crude tempo: energy-flux peak rate
            tempo = 120.0
            emotion = self._detect_emotion(energy, centroid, zcr, tempo)
            segments.append(AudioSegment(
                start_time=start_time, end_time=end_time, duration=end_time - start_time,
                energy=energy, tempo=tempo, emotion=emotion,
                beat_strength=0.0, spectral_centroid=centroid))
        print(f"Created {len(segments)} segments (WAV fallback)")
        return segments
...
class ScenePlanner:
    ...
    def __init__(self, style: str = "cinematic", theme: str = "nature", llm_client=None):
        self.style = style
        self.theme = theme
        self.scene_index = 0
        self.llm_client = llm_client  # optional llm_client.LLMClient for AI scripts

    def plan_scenes(self, segments: List[AudioSegment], llm_client=None) -> List[VideoScene]:
        """
        Plan video scenes for each audio segment

        Args:
            segments: List of audio segments
            llm_client: optional LLMClient; when configured, scene prompts are
                generated by a real LLM call, otherwise static style mappings
                are used (honest fallback, no key required).
        ...
        if llm is not None and getattr(llm, "configured", False):
            try:
                summary = "\n".join(
                    f"{i + 1}. {s.start_time:.1f}-{s.end_time:.1f}s emotion={s.emotion} "
                    f"energy={s.energy:.3f} tempo={s.tempo:.0f}"
                    for i, s in enumerate(segments))
                llm_prompts = llm.generate_scene_prompts(summary, self.style, self.theme, len(segments))
                print(f"AI script generated by LLM ({llm.model}) for {len(segments)} scenes")
            except Exception as e:
                print(f"LLM script generation failed ({e}); using static style mappings")
        elif llm is not None:
            print("LLM client has no API key; using static style mappings")
        ...
        for i, segment in enumerate(segments):
            if llm_prompts:
                ...
                negative_prompt = llm_prompts[i]["negative_prompt"]
                style_label = f"ai:{segment.emotion}"
            else:
                # Static emotion-based style mapping (no LLM)
...
class AudioToVideoGenerator:
    ...
    def __init__(self, style: str = "cinematic", theme: str = "nature",
                 use_mock: bool = False, llm_client=None):
        """
        ...
        Args:
            ...
            llm_client: optional llm_client.LLMClient for AI scene scripts
                (needs OPENAI_API_KEY; without it static mappings are used)
        """
        self.analyzer = AudioAnalyzer()
        self.planner = ScenePlanner(style=style, theme=theme, llm_client=llm_client)
